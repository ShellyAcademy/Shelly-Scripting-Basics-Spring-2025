function sendEmailNotification(totalPowerConsumption) {
    ...
    let receiver = { name: "Alex", email: "<email-address>" };
...
let previousHourTotalConsumption = 0;

function calcTotalPowerConsumption(){
  let switchData = Shelly.getComponentStatus("switch:0");
  
  let totalPowerConsumption = switchData.aenergy.total;
  let currentPowerConsumption = totalPowerConsumption - previousHourTotalConsumption;
  if (currentPowerConsumption > 1000){
    currentPowerConsumption /= 1000;
    if (currentPowerConsumption > 2.5){
      sendEmailNotification(currentPowerConsumption.toFixed(2));
    }
  }
  
  
  previousHourTotalConsumption = currentPowerConsumption;
}

calcTotalPowerConsumption();